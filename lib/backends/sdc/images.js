 ...
 * @param {Object} opts.req Restify req instance
...
function listImages(opts, callback) {
    ...
    var imgapi = opts.req.getHandle('imgapi');
    var log = opts.log;
    ...
    function listDockerImages(next) {
        ...
        Image.list(opts.req, log, params, function (err, imgs) {
            if (err) {
                next(err);
                return;
            }
        ...
        function getTags(img, cb) {
            ...
            ImageTag.list(opts.req, log, getParams, function (err, imgTag) {
                if (err) {
                    cb(err);
                    return;
                }
...
function imgFromImgInfo(opts, callback) {
    ...
    Image.list(opts.req, opts.log, filter, function (err, imgs) {
        if (err) {
            callback(err);
            return;
        } else if (!imgs.length) {
...
function imgFromName(opts, callback) {
    ...
    vasync.pipeline({funcs: [
        ...
        function findByName(_, next) {
            ...
            ImageTag.list(opts.req, log, filter, function (err, imgTags) {
                if (err) {
                    next(err);
        ...
        function findImage(_, next) {
            ...
            Image.list(opts.req, log, filter, function (err, imgs) {
        ...
        function delImgRef(_, next) {
            ...
            Image.del(opts.req, log, img, next);
        },
        function delImgTagRef(_, next) {
            ...
            ImageTag.del(opts.req, log, imgTag, next);
        }
...
function getScratchImage(req, callback) {
    ...
    var imgapi = req.getHandle('imgapi');
...
function deleteImage(opts, callback) {
    ...
    function verifyNotInUse(ctx, cb) {
        ...
        vmapi.listVms(query, {}, function (vmapiErr, vms) {
            if (vmapiErr) {
    ...
    function getDatacenterRefcount(ctx, cb) {
        ...
        Image.datacenterRefcount(opts.req, log, params, function (err, count) {
            if (err) {
                cb(new errors.DockerError(err, 'could not delete image'));
                return;
            }
    ...
    function untagHeads(ctx, cb) {
        ...
        vasync.forEachPipeline({
            ...
            func: function untagOne(imgTag, nextImgTag) {
                ...
                ImageTag.del(opts.req, log, imgTag, function (err) {
                    if (err) {
                        nextImgTag(err);
                    } else {
                        changes.push({
    ...
    function deleteImgs(ctx, cb) {
        ...
        function deleteOneImg(img, nextImg) {
            ...
            if (img.refcount > 1) {
                ...
                Image.update(opts.req, log, update, nextImg);
            } else {
                log.debug({imgId: img.docker_id, indexName: img.index_name},
                    'deleteImage: delete image', ctx.img.docker_id);
                Image.del(opts.req, log, img, function (delErr) {
                    if (delErr) {
                        nextImg(delErr);
                        return;
                    }
                    ...
                    imgapi.deleteImage(img.image_uuid, {}, function (err) {
...
function inspectImage(opts, callback) {
    ...
    imgFromName(opts, function (err, img) {
        if (err) {
            ...
            ImageTag.list(opts.req, opts.log, filter, function (err2, imgTags) {
                if (err2) {
                    callback(err2);
                    return;
                }
...
function pullImage(opts, callback) {
    ...
    var jobUuid;
    var wfapi = opts.req.getHandle('wfapi');
        ...
        if (err.code === 'ENOTFOUND') {
            ...
            payload = {
                status: format('Not Found (404) error from registry %s trying '
                    ...
                    opts.rat.remoteName, opts.req.getId())
            };
                ...
                if (code === 'MANIFEST_UNKNOWN') {
                    ...
                    payload = {
                        status: format('Error: image %s not found (%s)',
                            imgName, opts.req.getId())
                    };
                } else {
                    payload = {
                        status: format('Error pulling image: (%s) %s (%s)',
                            code, regErr.errors[0].message, opts.req.getId())
                    };
                }
            } else {
    ...
    function createPullJob(_, next) {
        ...
        wfapi.createPullImageJob(jobOpts, function (err, juuid) {
            if (err) {
                errorAndEnd(err);
                next();
                return;
            ...
            // progress back to us
            opts.sockets.setSocket('job', opts.rat.canonicalName, {
                socket: opts.res
            });
    ...
    function waitForPullJob(_, next) {
        common.waitForJob(wfapi, jobUuid, function (err, job) {
            if (err) {
                errorAndEnd(err, job);
            }
...
function createImgapiDockerImage(imageJson, opts, callback) {
    ...
    imgapi.adminImportImage(manifest, imageOpts,
...
function createImage(opts, callback) {
    ...
    var account_uuid = opts.req.account.uuid;
    ...
    function createSdcDockerImageLayer(ctx, next) {
        ...
        Image.create(req, req.log, img, next);
    }
}
...
function tagImage(opts, callback) {
    ...
    ImageTag.create(req, req.log, params, callback);
}
