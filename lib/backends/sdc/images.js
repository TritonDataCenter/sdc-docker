...
function listImages(opts, callback) {
    ...
    var dockerImages = [];
    // IMGAPI CLIENT
    var imgapi = opts.req.getClient('imgapi');
    ...
    function listDockerImages(next) {
        ...
        Image.list(opts.req.app, log, params, function (err, imgs) {
            if (err) {
                next(err);
                return;
            }
        ...
        function getTags(img, cb) {
            ...
            ImageTag.list(opts.req.app, log, getParams, function (err, imgTag) {
                if (err) {
                    cb(err);
                    return;
                }
...
function imgFromImgInfo(opts, callback) {
    ...
    Image.list(opts.req.app, opts.log, filter, function (err, imgs) {
        if (err) {
            callback(err);
            return;
        } else if (!imgs.length) {
...
function imgFromName(opts, callback) {
    ...
    vasync.pipeline({funcs: [
        ...
        function findByName(_, next) {
            ...
            ImageTag.list(opts.req.app, log, filter, function (err, imgTags) {
                if (err) {
                    next(err);
        ...
        function findImage(_, next) {
            ...
            Image.list(opts.req.app, log, filter, function (err, imgs) {
        ...
        function delImgRef(_, next) {
            ...
            Image.del(opts.req.app, log, img, next);
        },
        function delImgTagRef(_, next) {
            ...
            ImageTag.del(opts.req.app, log, imgTag, next);
        }
...
function getScratchImage(req, callback) {
    ...
    var imgapi = req.getClient('imgapi'); // IMGAPI CLIENT
    var log = req.log;
...
function deleteImage(opts, callback) {
    ...
    var req = opts.req;
    // VMAPI CLIENT
    var vmapi = opts.req.getClient('vmapi');
    var changes = [];
    ...
    function getImgTags(ctx, cb) {
        ...
        ImageTag.list(req.app, log, filter, function (err, imgTags) {
            if (err) {
                cb(err);
                return;
            }
    ...
    function getDatacenterRefcount(ctx, cb) {
        ...
        Image.datacenterRefcount(req.app, log, params, function (err, count) {
            if (err) {
                cb(new errors.DockerError(err, 'could not delete image'));
                return;
            }
    ...
    function untagHeads(ctx, cb) {
        ...
        vasync.forEachPipeline({
            ...
            func: function untagOne(imgTag, nextImgTag) {
                ...
                ImageTag.del(req.app, log, imgTag, function (err) {
                    if (err) {
                        nextImgTag(err);
                    } else {
                        changes.push({
    ...
    function deleteImgs(ctx, cb) {
        ...
        function deleteOneImg(img, nextImg) {
            ...
            if (img.refcount > 1) {
                ...
                Image.update(req.app, log, update, nextImg);
            } else {
                log.debug({imgId: img.docker_id, indexName: img.index_name},
                    'deleteImage: delete image', ctx.img.docker_id);
                Image.del(req.app, log, img, function (delErr) {
                    if (delErr) {
                        nextImg(delErr);
                        return;
                    }
                    ...
                    // IMGAPI CLIENT
                    var imgapi = req.getClient('imgapi');
...
function inspectImage(opts, callback) {
    ...
    imgFromName(opts, function (err, img) {
        if (err) {
            ...
            ImageTag.list(opts.req.app, opts.log, filter,
...
function pullImage(opts, callback) {
    ...
    var jobUuid;
    // WFAPI CLIENT
    var wfapi = opts.req.getClient('wfapi');
    ...
    function createPullJob(_, next) {
        ...
        wfapi.createPullImageJob(jobOpts, function (err, juuid) {
            ...
            // progress back to us
            opts.req.app.sockets.setSocket('job', opts.rat.canonicalName, {
                socket: opts.res
            });
...
function createImgapiDockerImage(imageJson, opts, callback) {
    ...
    log.debug({manifest: manifest, 'opts.rat': opts.rat},
        'createImgapiDockerImage');
