...
// Note that this differs from the Swarm one in expr.go to fix some issues
// and accomodate slight parsing differences.
// JSSTYLED
 ...
 * to the docker if a container has both 'foo' and 'docker:label:foo' VM tags.
 *
...
function _vmUuidsFromAffinity(opts, cb) {
    ...
    // consider name matching.
        ...
        opts.vmapi.getVm({
            ...
            {
                /*
                 * If the 'docker:id' check fails, treat this like a 404.
                 * This could happen if someone made up a docker_id that
                 * started with the 32-chars for a real VM.
                 */
                cb(null, [vmUuid]);
            } else {
                cb(new errors.ResourceNotFoundError(format(
                    'no active container found with id "%s" for affinity "%s"',
    ...
    // filter function to select a match.
        ...
        vasync.pipeline({funcs: [
                ...
                getAllActiveVms(function (err, allVms) {
                    ...
                    case 'exact':
                        ...
                        if (nameMatch) {
                            ...
                            // If more than one, then ambiguous: treat like 404.
            ...
            if (err) {
                ...
                cb(new errors.ResourceNotFoundError(format(
                    'no active container(s) match %s for affinity "%s"',
                    aff.value, _strFromFilterExpr(aff))));
            } else {
                log.debug({affinity: _strFromFilterExpr(aff), vms: vms},
                    '_vmUuidsFromAffinity');
 ...
 * - A "container" affinity <value> is always meant to resolve to one or more
 *   active VMs. If no match is found, then an error is returned.
 * - A tag affinity <value> might not match any active VMs. If it doesn't
 *   *and the operator is `==`* then we fail. (If we didn't fail, then we'd
 *   end up setting no `locality` and the strict affinity would be blithely
 *   ignored.)
 *
        ...
        func: function setLocalityFromAff(aff, next) {
            if (aff.key === 'image') {
                ...
                _vmUuidsFromAffinity({
                    ...
                    if (err) {
                        next(err);
                    } else if (vmUuids.length === 0 && strict
                        && aff.key !== 'container' && aff.operator === '==')
                    {
                        /*
                         * An error case: (a) a tag/label-affinity,
                         * (b) using the '==' operator, (c) that is strict,
                         * and (d) matches no VMs.
                         */
                        next(new errors.ResourceNotFoundError(format(
                            'no active containers found matching tag "%s=%s" '
                            + 'for affinity "%s"',
