...
/*
 * This defines the workflow job that handles pulling images. It runs in the
 * context of workflow so it doesn't follow the conventions in the rest of
 * sdc-docker. For example, it can't use req.getHandle() since that's running
 * under wf-runner, not our Restify app.
 */

// Doing these require()s is not really needed since the code's not even going
// to run here, but javascriptlint will complain if these aren't defined.
