...
function vmobjToContainer(opts, obj, fwrules, callback)
{
    ...
    } else if (obj.state == 'provisioning' && obj.create_timestamp) {
        uptime = Math.floor((now - (new Date(obj.create_timestamp))) / 1000);
        container.Status = 'Provisioning ' + common.humanDuration(uptime);
 ...
 * "Env" and "Labels" entries, if any, in a "docker run" API call.
 *
 * See design in DOCKER-630. XXX RFD this
 *
 * See: https://github.com/docker/swarm/blob/master/cluster/config.go#L80-L107
 * Because currently Docker Swarm filter docs don't mention labels, but the
 * code does.
 */
function localityFromContainer(opts, cb) {
    ...
    // XXX remove those entries from env
    // XXX parse out the other filters from env? Even if ignoring them.
    // XXX update 'affinities' label with the merged value
    // XXX doc these side-effects on opts.container!

    // Limitation: can't handle mixed hard and soft affinities.
    var allIsSoft;
    for (var i = 0; i < affinities.length; i++) {
        var isSoft = affinities[i].isSoft;
        if (allIsSoft === undefined) {
            allIsSoft = isSoft;
        } else if (isSoft !== allIsSoft) {
            cb(new errors.NotImplementedError(format(
                'mixed hard and soft affinities, "%s" and "%s"',
                _filterExprToString(affinities[0]),
                _filterExprToString(affinities[i]))));
            return;
        }
    }

    var strict = !allIsSoft;
    var near = [];
    var far = [];

    // XXX Really want forEachParallel with concurrency
    vasync.forEachPipeline({
        inputs: affinities,
        func: function setLocalityFromAffinity(affinity, next) {
            if (affinity.key === 'image') {
                // XXX see DOCKER-630 notes for handling image affinity
                XXXimage
                next();
            } else if (affinity.key === 'container') {
                // Find the UUID(s) for the given instance name/id or glob.
                //XXXcontainer
                (affinity.operator === '==' ? near : far).push(affinity.value);
                next();
            } else {
                // Find the UUID(s) for containers matching the given label.
                XXXlabel
            }
        }
    }, function (err) {
        ...
        var locality = {};
        if (strict) locality.strict = true;
...
function _affinitiesFromContainer(opts) {
    ...
    if (labels && labels['com.docker.swarm.affinities']) {
        // XXX This can throw. What's Docker behaviour with bad label?
        affinities = affinities.concat(
            _affinitiesFromLabel(labels['com.docker.swarm.affinities']));
    }

    // Env, e.g.: [ 'affinity:foo==bar' ]
    var env = opts.container.Env;
    var i, kv, parts, affinity;
    for (i = 0; i < env.length; i++) {
        kv = env[i];
        if (kv.slice(0, 9) === 'affinity:') {
            parts = strsplit(kv, ':', 2);
            // XXX Can throw. What's Docker behaviour with bad affinit env?
            affinities.push(_parseFilterExpr(parts[1]));
        }
    }
...
function _parseFilterExpr(s) {
    ...
    for (var i = 0; i < operators.length; i++) {
       ...
       console.log('XXX expr.value', expr.value);
...
function _filterExprToString(expr) {
    return format("%s%s%s%s", expr.key, expr.operator, expr.isSoft ? '~' : '',
        expr.value);
}

/**
 * Parse an affinity object from a `docker run` "com.docker.swarm.affinities"
 * label.
 *
 * Throws if there is an error parsing.
 */
function _affinitiesFromLabel(label) {
    assert.string(label, 'label');

    var arr;
    try {
        arr = JSON.parse(label);
    } catch (parseErr) {
        // XXX What is Docker-docker behaviour here?
        throw parseErr;  // XXX better details in error msg
    }
    ...
    for (var i = 0; i < arr.length; i++) {
        affinities.push(_parseFilterExpr(arr[i]));
    }
