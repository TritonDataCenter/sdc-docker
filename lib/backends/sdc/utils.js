...
function _containerStatusFromVmobj(opts) {
    ...
    if (vmobj.state == 'running') {
            ...
            && vmobj.hasOwnProperty('exit_timestamp'))
        {
            ...
            /*
             * Two known possibilities here:
             * 1. The zone never started (e.g. if created, but not run; or
             *    some failure before it got to running). Last sometime checked
             *    Docker-docker returns the empty string for this.
             * 2. The CN crashed or hard-stopped such that zoneadmd couldn't
             ...
             *    for this.
             *
             * "Exited" isn't really correct here if it never started, so let's
             * not lie and just say "Stopped". We are already going to differ
             * from Docker-docker anyway.
             */
            status = 'Stopped';
        }
    } else {
        // TODO: handle other states here!
        status = '';
    }
